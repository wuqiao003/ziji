...
def load_dataset(data_dir: str, dataset_name: str) -> Dict[str, Any]:
    ...
    with open(os.path.join(data_dir, f'{dataset_name}.item.json'), 'r', encoding='utf-8') as f:
        data['items'] = json.load(f)
    
    # Load item_id to semantic tokens mapping from index.json
    with open(os.path.join(data_dir, f'{dataset_name}.index.json'), 'r', encoding='utf-8') as f:
