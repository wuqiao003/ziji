...
def train(
    ...
    category_dict = {"Industrial_and_Scientific": "industrial and scientific items", "Office_Products": "office products", "Toys_and_Games": "toys and games", "Sports": "sports and outdoors", "Books": "books", "Edu_Questions": "exercise questions"}
    print(category)
    ...
    # ===== 教育场景：构建 题目SID -> 难度 映射，供 zpd_reward 估计「最近发展区」=====
    sid2difficulty = {}
    if reward_type in ("zpd", "ranking_zpd"):
        with open(item_meta_path, 'r', encoding='utf-8') as f:
            _item_meta = json.load(f)
        with open(sid_index_path, 'r', encoding='utf-8') as f:
            _sid_index = json.load(f)
        for _iid, _sids in _sid_index.items():
            if _iid in _item_meta and len(_sids) >= 1:
                _combined = "".join(_sids)
                sid2difficulty[_combined] = float(_item_meta[_iid].get('difficulty', 0.5))
        print(f"[ZPD] Loaded difficulty for {len(sid2difficulty)} questions.")
    ...
    def zpd_reward(prompts, completions):
        """教育场景 reward：奖励落在学生「最近发展区(ZPD)」内的题目。

        - 学生当前能力 ability：用其历史练习题的难度均值估计；
        - 推荐题难度 diff 越接近 ability，奖励越高；
        - 难度差 gap 落在 ZPD 窗口(<=0.15)内额外加分，鼓励「跳一跳够得着」。
        约束解码已保证 completion 基本都是真实存在的题 SID。
        """
        rewards = []
        for i, comp in enumerate(completions):
            sid = comp.strip("\n\" ")
            # 用历史练习题难度均值估计学生能力
            hist = prompt2history.get(prompts[i], "")
            hist_sids = [h.strip() for h in hist.split("::") if h.strip()]
            hist_diffs = [sid2difficulty[h] for h in hist_sids if h in sid2difficulty]
            ability = sum(hist_diffs) / len(hist_diffs) if hist_diffs else 0.5
            if sid not in sid2difficulty:
                rewards.append(0.0)
                continue
            diff = sid2difficulty[sid]
            gap = abs(diff - ability)
            r = max(0.0, 1.0 - gap / 0.5)   # 难度=能力时最高(1.0)，偏离越远越低
            if gap <= 0.15:                 # 命中 ZPD 窗口，额外加分
                r += 0.3
            rewards.append(r)
        return rewards
    ...
    elif reward_type == "zpd":
        reward_fun = zpd_reward
    elif reward_type == "ranking_zpd":
        reward_fun = [rule_reward, zpd_reward]
