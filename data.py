...
class SidDataset(CSVBaseDataset):
    ...
    def get_history(self, row):
        ...
        return {"input": f"The student has practiced the following exercises {history} in chronological order. Recommend the next exercise (its semantic ID) that best fits the student's current learning progress.",
                # Analyze the student's mastery and then predict the semantic ID of the next exercise.
                "output": target_item + "\n",
                "history_str": history_str,
...
"""
    ...
    def get_history(self, row):
        ...
        last_history_item_sid = row['history_item_sid'][-1] if row['history_item_sid'] else None
        return {"input":
                f"The student has practiced the following exercises {history} in chronological order. Recommend the next exercise (its semantic ID) that best fits the student's current learning progress.",
                "output": target_item + '\n',
                "dedup": target_item_sid == last_history_item_sid}
